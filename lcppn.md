...
int (*pfce)( const char*, ... ) = printf;    // 可以指向 printf()
int (*pfc)( const char* ) = strlen;          // 可以指向 strlen()
```
...
如果基类的函数前没有 `virtual`，那么尽管派生类尝试重写这个函数，但是调用时仍然调用的是基类的函数。比如，在 `Zhiyu` 类中有个函数叫做 `grow_up()`，但是这个函数并没有声明为 `virtual`，而它的派生类 `Moke` 尝试“重写”了这个函数，那么最终在调用 `Moke` 类调用的 `grow_up()` 是 `Zhiyu` 类的 `grow_up()` 而不是 `Moke` 类“重写”的 `grow_up()`。
...
另外，**最好把基类的析构函数声明为虚函数**。即使基类并不需要析构函数，我们也可以定义一个函数体为空的虚析构函数，以保证撤销动态分配空间能正确的处理。一般基类的析构函数不应该是 `protected`。
...
本来的想法是，在 `Zhiyu` 类中调用 `fun()` 会传入默认实参 `1024`，而在 `Moke` 类中调用 `fun()` 会传入默认实参 `1210`。
